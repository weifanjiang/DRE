...
# %%
def scout_convert_to_feature_vector_max(train_df, test_df, granularity=None, objective='incidentRouting'):
    ...
    if granularity is None:
        ...
        for keys, sub_df in grb_gran:
            aggregated_result = aggregation_based_reduction(sub_df[grb_cols], dir="row", grb='IncidentId', option=1)
    ...
    train_vals = train_vals[train_label != -1, :]
    train_label = train_label[train_label != -1]
    test_vals = test_vals[test_label != -1, :]
...
# %%
if dummy:
    save_dir = os.path.join(scout_data_dir, 'redundant')
else:
...
for location in ['all']:
    ...
    if not os.path.isfile(os.path.join(save_dir, "{}_data.pickle".format(location))):
        ...
        train_vals, train_label, test_vals, test_label = scout_convert_to_feature_vector_max(
            train_part, test_part
        )

        with open(os.path.join(save_dir, "{}_data.pickle".format(location)), "wb") as fout:
...
for location in ['all']:
    ...
    eval_dir = os.path.join(save_dir, location)
    os.system("mkdir -p {}".format(eval_dir))
    for at in automl_times:
        out_path = os.path.join(eval_dir, "{}.json".format(at))
        if not os.path.isfile(out_path):
            # train model and run evaluation
            automl = autosklearn.classification.AutoSklearnClassifier(
                time_left_for_this_task=at,
                n_jobs=8,
                per_run_time_limit=at//6,
                memory_limit=scout_automl_mem
            )

            automl.fit(X_train, Y_train)
            Y_test_pred = automl.predict(X_test).astype(int)
